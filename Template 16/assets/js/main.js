...
(function () {
  ...
  /**
   * SoolPair Auth Status Sync for GNB Navigation
   */
  const syncAuthNavigation = () => {
    ...
    if (loginNav) {
      ...
      if (isLoggedIn) {
        loginNav.textContent = '로그아웃';
        loginNav.href = '#';
        // Remove duplicate listeners if any, by cloning or checking status
        loginNav.onclick = (e) => {
          e.preventDefault();
        ...
        loginNav.textContent = '로그인';
        loginNav.href = 'login.html';
        loginNav.onclick = null;
      }
    }
  };
  window.addEventListener('DOMContentLoaded', syncAuthNavigation);
  window.addEventListener('load', syncAuthNavigation);
